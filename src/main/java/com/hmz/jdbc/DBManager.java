...
public class DBManager {
	 ...
	 * <br>....
	 * <br>Note : This Method is not for use of INSERT, DELETE, UPDATE operations !*/
	public ResultSet executeQuery(String sql) throws SQLException {
		return statement.executeQuery(sql);
	}
		
	/** GENERAL STATEMENTS INSERT, UPDATE, DELETE
	 * 
	 * @param sql
	 * @return
	 * @throws SQLException
	...
	public int s_insertInto(String table, String ...values) throws Exception {
		...
		for(int i = 1; i <= values.length; i++) {
			if(values[i - 1].equals("null")) {
				cps.setObject(i, null);
			}
			else cps.setString(i, values[i - 1]);
		}
		return cps.executeUpdate();
	}
	//----------------------------------------------------------------------------------------------------------------------------	
